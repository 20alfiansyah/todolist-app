...
const CreateModal = ({handleModal, handleNotify}) =>{
    ...
    const handleForm = (e) => {
        const { target } = e
        formik.setFieldValue(target.name, target.value == 'false' ? false : target.value)
    }
    return (
        <AnimatePresence>
            <motion.div  initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}  className="w-full h-full absolute bg-black bg-opacity-35 flex flex-col justify-center items-center gap-2 ">
                ...
                <div className="w-10/12 lg:w-2/5 px-5 py-7 bg-white rounded-lg flex flex-col gap-2">
                    <div>
                        <h1 className='font-bold text-lg'>Create Your <span className='text-primary'>Activity</span></h1>
                        <hr  className='border-1 rounded-md border-opacity-45 border-base-100'/>
                    </div>
                    <div>
                        <form action="http://localhost:5000/api" method='POST' onSubmit={formik.handleSubmit}>
                            <label htmlFor="" className='font-semibold'>Title</label>
                            <input type="text" name='todo' onChange={handleForm}  className='w-full border-2 rounded-md border-base-100 border-opacity-20 p-2'/>
                            ...
                            <motion.button  type='submit'  whileTap={{ scale: 0.85 }} className="w-full p-3 lg:text-sm bg-base-100 text-white font-bold rounded-lg">ADD TASK <FontAwesomeIcon icon={faPlus}/></motion.button> 
                        </form>
                    </div>
                </div>
            </motion.div>
